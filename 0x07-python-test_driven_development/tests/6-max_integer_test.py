...
class TestMaxInteger(unittest.TestCase):
    ...
    def test_types(self):
        ...
        with self.assertRaises(TypeError):
            max_integer([1, 2, '6', 4.0], 4.0)
        with self.assertRaises(TypeError):
            max_integer(["1234", 'z'], '4')
    ...
    def test_max(self):
        ...
        self.assertEqual(max_integer([i for i in range(10**6)]), 999999)
        ...
        def test_lists(self):
            """confirns list inputs"""
            self.assertEqual(max_integer(['name', 'banana']), 'name')
